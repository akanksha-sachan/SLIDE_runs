...
FillDataFrame <- function(df1, features, list_CD4) {
  ...
  for (j in 1:length(features)) {
    ...
    if (length(list_per) != length(list_CD4)) {
      stop("percentage list length is off ... ")
    }
...
for (f in features) {
  if (f %in% row.names(CD4_obj) == FALSE) {
    stop("wrong feature names")
  }
}
